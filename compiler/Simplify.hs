...
subst g ts@((FI.Record (l, _)):_) = FI.Record (l, t')
  where t' = subst g . map (\x -> let FI.Record (_, t) = x in t) $ ts
--subst g ((FI.Datatype _ _):_) =
--subst g ((FI.ListOf _):_) =
...
dedeBruE i as j xs (Case e alts)                  = Case (dedeBruE i as j xs e) (map dedeBruijnAlt alts)
  where dedeBruijnAlt (ConstrAlt (Constructor name ts) names fe) =
          ConstrAlt 
            ...
            (\xs' -> dedeBruE i as (j + n) ((reverse xs') ++ xs) (fe [j..j + n - 1])) where n = length ts
